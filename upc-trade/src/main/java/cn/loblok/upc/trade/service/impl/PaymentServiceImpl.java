...
import cn.loblok.upc.trade.strategy.AliPayService;
import cn.loblok.upc.trade.strategy.WeChatPayService;
...
public class PaymentServiceImpl implements PaymentService {
    ...
    public PaymentStatusResponse getPaymentStatus(String orderId) {
        ...
        // 如果本地已经(PAID / FAILED)，直接返回
        if (MembershipOrderStatus.PAID.equals(order.getStatus()) || MembershipOrderStatus.FAILED.equals(order.getStatus())) {
        ...
        if (!remoteStatus.equals(order.getStatus())) {
            order.setStatus(MembershipOrderStatus.valueOf(remoteStatus));
            if (MembershipOrderStatus.PAID.getValue().equals(remoteStatus)) {
                order.setPaidTime(LocalDateTime.now());

                orderService.deliveryMember(order);
    ...
    private String queryThirdPartyStatus(TOrder order) {
        ...
        return MembershipOrderStatus.PAYING.getValue(); // 默认保持原样
    }
