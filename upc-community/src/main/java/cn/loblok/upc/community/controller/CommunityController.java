...
public class CommunityController {
    ...
    public Result<List<CommunityUserVO>> getRecommendFollowList(@CurrentUser Long userId) {
        try {
            ...
            List<CommunityUserVO> resultList = recommendUsers.stream().map(
                    user -> {
                        CommunityUserVO communityUserVO = new CommunityUserVO();
                        BeanUtils.copyProperties(user, communityUserVO);
                        communityUserVO.setIsFollowed(false);
    ...
    public Result<CommunityUserVO> getUserProfile(@PathVariable("targetUserId") Long targetUserId,
        ...
        try {
            CommunityUserVO userPublicInfoDTOInfo = communityService.getUserProfile(targetUserId, currentUserId);
            return Result.success(userPublicInfoDTOInfo);
        } catch (Exception e) {
    ...
    public Result<List<CommunityUserVO>> searchUsers(@CurrentUser Long userId, @RequestParam("q") String keyword,
        ...
        try {
            List<CommunityUserVO> searchResults = communityService.searchUsers(keyword, page,pageSize,userId);
